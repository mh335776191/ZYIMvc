...
namespace ZY.Extensions
{
    ...
    public class DawnValidator
    {
        ...
        {
            if (string.IsNullOrEmpty(strVerify)) return false;
            return Regex.IsMatch(strVerify, "^[\u4E00-\u9FA5]{2,4}$");
        }
        /// <summary>
        /// 判断字符串是否由中文组成
        /// </summary>
        ...
        /// <param name="strVerify">需要验证的字符串</param>
        /// <returns>True/False，符合/不符合</returns>
        public static bool TelIsPhone(string strVerify)
        {
            if (string.IsNullOrEmpty(strVerify)) return false;
        ...
        {
            if (string.IsNullOrEmpty(strVerify)) return false;
            string[] userip = DawnStringRelated.SplitString(strVerify, @".");
            for (int ipIndex = 0; ipIndex < limitArray.Length; ipIndex++)
            {
                string[] tmpip = DawnStringRelated.SplitString(limitArray[ipIndex], @".");
                int r = 0;
        ...
        {
            if (string.IsNullOrEmpty(strVerify)) return false;
            if (DawnStringRelated.GetOccurrenceNumber(strVerify, "#") > 1) return false;
        ...
        {
            if (String.IsNullOrEmpty(strVerify) || strVerify.Length != 36) { return false; }
            string[] arr = strVerify.Split('-');
